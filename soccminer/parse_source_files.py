...
class SourceFiles:
    ...
    def convert_srcfiles_to_xmlfiles(self, project_name, exception_obj):
        logging.info("source code to xml file conversion begins with current dir {}".format(os.getcwd()))
        try:
            temp_dir_loc = None
            if Platform.is_unix_platform():
            ...
            logging.error("Unable to create temp folder to store xml files ~ Process failed")
        ...
        for code_file in self.files:
            ...
            try:
                ...
                try:
                    ...
                    logging.error("Error occured while conversion to xml for: {}, ERROR: {}".format(code_file, error_message))
                    continue
                else:
                    ...
                    if ret_code == 0:
                        ...
                        logging.error("AST conversion failed with {}:{} for {}".format(ret_code,message,code_file))
        ...
        if len(failed_files) > 0:
            exception_obj.update_exception_message(project_name,
                                                   "Project {} has {} failed XML conversions. Check Log file for details".format(project_name, len(failed_files)))
